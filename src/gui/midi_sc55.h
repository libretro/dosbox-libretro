 ...
 *  From DOSBox Pure (https://github.com/schellingb/dosbox-pure); here the ROMs
 *  are opened from the host rather than from its mounted drives.
 *
...
struct MidiHandler_sc55 : public MidiHandler
{
	...
	// A ROM file on the host, as a DOS file, with its size. (DOSBox Pure opens
	// them through FindAndOpenDosFile, which can reach its mounted drives.)
	static DOS_File* OpenHostROM(const char* path, Bit32u* size)
	{
		FILE* f = fopen_wrap(path, "rb");
		if (!f) return NULL;
		DOS_File* df = new rawFile(f, false);
		df->AddRef();
		Bit32u end = 0, zero = 0;
		df->Seek(&end, DOS_SEEK_END);
		df->Seek(&zero, DOS_SEEK_SET);
		*size = end;
		return df;
	}

	// conf is where the ROMs are: a ZIP holding them, the path of ROM1.BIN, or
	// a directory with ROM1.BIN in it (or in an SC-55 directory in it). The
	// other ROMs sit next to ROM1.BIN: ROM2, ROM2_ST, ROM_SM and WAVEROM1-3,
	// with the same capitalization and extension.
	bool Open(const char * conf)
	{
		if (!conf || !*conf) return false;
		std::string confstr(conf);
		int conf_len = (int)confstr.size();
		if (conf_len > 4 && !strcasecmp(conf + conf_len - 4, ".zip"))
		{
			FILE* zip_file_h = fopen_wrap(conf, "rb");
		...
		{
			if (conf_len < 8 || strcasecmp(conf + conf_len - 8, "ROM1.BIN"))
			{
				// A directory: find ROM1.BIN in it, or in its SC-55 directory.
				const char* candidates[] = { "SC-55/ROM1.BIN", "SC-55/rom1.bin", "ROM1.BIN", "rom1.bin" };
				bool found = false;
				for (const char* c : candidates)
				{
					std::string p = confstr;
					if (!p.empty() && p.back() != '/' && p.back() != '\\') p += '/';
					p += c;
					struct stat st;
					if (host_stat(p.c_str(), &st) == 0) { confstr = p; found = true; break; }
				}
				if (!found) return false;
				conf = confstr.c_str();
				conf_len = (int)confstr.size();
			}

			Bit32u rom1_size;
			roms[NROM1] = OpenHostROM(conf, &rom1_size);
			...
			for (const char* romfn : romfns)
			{
				...
				Bit32u romsize;
				DOS_File* f = OpenHostROM(path, &romsize);
				if (!f) continue;
