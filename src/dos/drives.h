...
// ZIP, memory and union drives from DOSBox Pure (https://github.com/schellingb/dosbox-pure),
// and the helpers they share. A ZIP file is mounted read-only as a zipDrive,
// and a unionDrive over it keeps whatever the game writes in a save file of
// its own, so the archive itself is never modified.
#ifndef DBP_ASSERT
#define DBP_ASSERT(cond)
#endif
#ifndef DBP_STATIC_ASSERT
#define DBP_STATIC_ASSERT(cond) static_assert(cond, #cond)
#endif
#ifndef OPEN_IS_WRITING
#define OPEN_IS_WRITING(flags) (flags & 3)
#define OPEN_IS_READING(flags) (!(flags & 1))
#endif
#ifndef OPEN_CHECK_ACCESS_CODE
#define OPEN_CHECK_ACCESS_CODE(flags) ((flags&0xf) <= 4)
#endif
#ifndef DOSERR_DRIVE_NOT_READY
