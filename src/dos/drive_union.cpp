	...
	static void WriteSaveFile(Bitu implPtr)
	{
		...
		if (!fsave)
		{
			...
			if (reportcount++ < 3 || !(reportcount % 6))
			{
				...
				LOG_MSG("Error while writing game save file '%s'!", impl->save_file.c_str());
			}
		...
		for (SaveFile& sf : save_files)
		{
			...
			if (under_match_size || is_swap)
			{
				...
				if (is_swap)
				{
					...
					LOG_MSG("Game is writing %d MB swap file '%s'", size / 1024 / 1024, path);
					#endif
				}
			}
...
{
	dirCache.SetLabel(under.GetLabel(), false, true);
}
